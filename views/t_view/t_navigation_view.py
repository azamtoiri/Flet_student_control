...
from utils.customs import STContainer as TContainer


class Containers:
    def __init__(self):
        # home container
        self.home_container = TContainer(Text('Учительская', color=colors.BLACK), alignment=alignment.center)

        # courses container
        self.courses_container = TContainer(content=Text('Курсы', size=14, color=colors.BLACK),
                                             alignment=alignment.center)
        self.grades_container = TContainer(content=Text('Оценки', size=14, color=colors.BLACK),
                                            alignment=alignment.center)
        self.tasks_container = TContainer(content=Text('Задания', size=14, color=colors.BLACK),
                                           alignment=alignment.center)
        self.profile_container = TContainer(Text('Профиль', color=colors.BLACK), alignment=alignment.center)


class TNavigationView(View, Containers):
    def __init__(self) -> None:
        super().__init__()
        self.route = '/teacher/main'
        #
        # Button
        #
